...
| **#39** | feat: xuanji-Dawn 文档提取集成 via WuKongIM bridge | ✅ 已迁移 | P3 | 依赖旧 `zeroclaw-channel-wukongim` crate 和 `XUANJI_BRIDGE` mpsc 机制；需与 #46 捆绑，按 0.8.0 的 `dawn_im` 模块架构重写 | 已迁移完成（与 #46 捆绑实施），架构升级：`parking_lot::RwLock` 替代 `OnceLock` 支持 hot reload；`TaskMessage` struct 替代裸 tuple；多 dawnim 别名路由；listener 每条消息独立 spawn 隔离失败 |
