...
| **#51** | Improve status update handling and clean up sender history（含 2 commits：sentinel 清理 + status_update fire-and-forget） | 🔶 部分迁移 | P2 | **Commit 1 `be488c1cf`（channels: strip sentinel assistant turns）**：0.8.0 `orchestrator/mod.rs` 仍保留三种 sentinel 字符串（"Task failed/timed out/Session interrupted — not continuing this request"，行 5929/5979/10116）写入路径，但**未做读出端清理**。`prior_turns` 构造点位于行 4743（`normalize_cached_channel_turns` 之后），新清理块可直接插入在 image 剥离（行 4765）之前。改动与 channel 无关，影响所有 dawn_im / acp / 其他 channel，属 history poisoning 止血修复，成本低（+~30 行 + 单元测试）。**Commit 2 `8d5ae4e23`（wukongim: status_update fire-and-forget）**：0.8.0 `Channel` trait 无 `send_status_update` 方法；`DawnIMConfig.progress_streaming` 字段在 `schema.rs:12847` 注释明确 "Currently unwired in the 0.8.0 port — reserved for future re-implementation"。**进一步核实**：`DawnIMChannel.progress_streaming` 字段虽存在于 struct（`dawn_im/channel.rs:122`）且从 config 拷贝（行 164），但全代码库**零读取点** —— 没有 `if self.progress_streaming { ... }` 分支，没有 `send_status_update` 调用方，整条状态流式上报链路尚未接回。bug 不存在（因代码路径不存在），fire-and-forget 策略需在重接时一并设计 | **Commit 1 待迁移**：直接移植 SENTINELS 数组 + while 循环到 `prior_turns` 处理段，附原 PR 的语义（删除 sentinel 同时回收前置 user turn 保证 user/assistant 交替）。**Commit 2 暂缓**：等 0.8.0 status streaming surface 重新接入时一并采纳 fire-and-forget 设计 |
| **#52** | Refactor outbound message handling and improve heartbeat detection（含 2 commits） | ❌ 未迁移 | P2 | **Commit 1 `a933837a5`（tighten heartbeat detection）**：0.8.0 `dawn_im/channel.rs` 心跳循环（行 1486–1503）存在与 master wukongim 同款 bug —— `last_activity = Instant::now()`（行 1506）在 `let WsMsg::Text(text) = frame else { continue; };`（行 1507）**之前**执行，导致 TCP 控制帧（Ping/Pong/Binary）刷新活跃度，server 静默时无法及时检测出僵尸连接。原 PR 还移除了 `send_ws_frame(WsMsg::Ping)` —— 0.8.0 dawn_im 心跳循环（行 1483+）只发 JSON-RPC ping，**无需该项移植**。HEARTBEAT_TIMEOUT 90→60 适用于 `dawn_im/connection.rs:12`（但被 #53 进一步收紧到 25s，**合并实施时跳过中间值,直接 90→25**）。**Commit 2 `4aecb777c`（replace send_rpc("send") with fire-and-forget）**：0.8.0 `dawn_im/channel.rs` 5 处沿用 `send_rpc("send", params)` 等 30s ACK 的反模式（行 853 `send_with_topic` / 951 `send_task_payload` / 1354 `send_status_message_with_topic` / 1598 `request_approval` / 1636 `cancel_approval`）。所有 5 处都 `let _: serde_json::Value = ...` 弃用响应,真正的业务响应走 `pending_responses`/`pending_approvals` 独立 oneshot,故全部安全。WuKongIM/Dawn 服务器实际不可靠回 send ACK,导致 approval 卡片在用户响应前就 RPC timeout（与 #50 thread routing 修复呼应）。**注意**：`DawnIMChannel::send` 的 `SendKind::Text` 分支（行 1181-1247）**已经是直接 WS Text 发送**（无 `send_rpc`）,形态与 master 修复后的 wukongim `Channel::send` 一致,**不需要再改**;可选项是把这 5 处与 `Channel::send` 共享一个 `send_params_fire_and_forget` 辅助函数(DRY 优化,非 bug 修复必需) | **Commit 1 部分迁移**：移动 `last_activity = Instant::now();` 到 `let WsMsg::Text(...) else { continue; };` 之后（结构性 bug 修复），数值调整跟 #53。**Commit 2 待迁移**：dawn_im 5 处 `send_rpc("send", ...)` 切换为 fire-and-forget,注意 `request_approval`/`cancel_approval` 已绑定 `pending_approvals` oneshot 等待用户响应,仅 RPC 帧本身改为不等 ACK 即可 |
| **#53** | Enhance WebSocket stability with TCP keepalive and heartbeat adjustments | ❌ 未迁移 | P2 | 4 项独立改动：（a）**TCP keepalive**（10s idle / 5s interval）—— 0.8.0 `dawn_im/channel.rs:1377-1382` 用 `tokio_tungstenite::connect_async` 直连。原 PR 走 `socket2::Socket::new` → 手工 `connect` → `from_std` → `client_async` 路径（+~20 行 + `socket2` 依赖）；**0.8.0 上可优先尝试更轻量方案**：`connect_async` 之后用 `socket2::SockRef::from(&tcp_stream)` 对底层 TCP 设 `set_tcp_keepalive`（+~5 行,不重写 DNS/connect 流程），需验证 (i) 当前 `tokio-tungstenite` 版本是否暴露底层 `TcpStream` 引用; (ii) TLS 包装（`MaybeTlsStream::Tls`）下能否拿到底层 TCP。失败再回退到原 PR 方案。（b）**PING_INTERVAL 30→10 + HEARTBEAT_TIMEOUT 90→25**（与 #52#1 合并跳过 60 中间值）适用于 `dawn_im/connection.rs:11-12`。（c）**Backoff 稳定运行重置** 适用于 `orchestrator/mod.rs:4170+` `spawn_supervised_listener_with_health_interval`：0.8.0 `Ok(())` 分支（行 4230）已重置 backoff,但 `Err(_)` 分支（行 4250-4261）不重置,导致长跑后掉线时仍需等指数退避。在 `let result = { ... };`（行 4200-4213）之前记 `start_time`,在 `match result { ... }` 之前判断 `start_time.elapsed() >= 30s` 时重置 `backoff = initial_backoff_secs.max(1)`,语义上等同于"`Err(_)` 也跟着重置"。（d）**LocalTimer with microsecond precision** —— master 改在 `src/main.rs::init_logging`；0.8.0 已将 tracing 配置统一搬至 `crates/zeroclaw-log/src/subscriber.rs`,`AgentAliasFormatter::inner` 当前持有 `fmt::format::Format<Full, fmt::time::SystemTime>`（行 102）,把 `SystemTime` 替换为基于 `chrono::Local` 的自定义 `FormatTime` 即可。**`chrono` 已是 `zeroclaw-log` 直接依赖**（`Cargo.toml:12`,含 `clock`/`std`/`serde` features）,无需新增 dep | 待整体迁移：4 项均与 channel 解耦或可干净适配,作为 dawn_im 网络稳定性合并补丁推进,建议与 #52#1（last_activity 修复）一起出 PR |
...
2. **P2 — #41 / #48 / #51 / #52 / #53**：
   ...
   - **#52 commit 1（last_activity 结构性修复）+ #52 commit 2（send_rpc → fire-and-forget）+ #53（TCP keepalive + 心跳收紧 + backoff 重置 + LocalTimer）** 建议作为 "dawn_im 网络稳定性合并补丁" 一次性出 PR,避免心跳数值/调度顺序之间留下中间态。
     - **PR 内建议拆 5 个 commit 便于 review**:
       1. `fix(dawn_im): tighten heartbeat detection — fix last_activity placement + bump to 10s/25s`(#52#1 结构修复 + #53 心跳参数,跳过 60s 中间值)
       2. `fix(dawn_im): replace send_rpc("send") with fire-and-forget for 5 outbound paths`(#52#2,5 处明确列出)
       3. `feat(dawn_im): enable TCP keepalive on WebSocket socket`(#53#a,先试 `SockRef::from` 轻量方案)
       4. `fix(channels): reset reconnect backoff after stable runtime ≥30s`(#53#c)
       5. `feat(log): switch fmt timer to local timezone with microsecond precision`(#53#d,在 `zeroclaw-log/subscriber.rs` 改,复用已有 chrono)
