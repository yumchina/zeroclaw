...
impl SecurityPolicy {
    ...
    pub fn prompt_summary(&self) -> String {
        ...
        // Workspace constraint + allowed roots.
        //
        // Merge `workspace_dir` and `allowed_roots` into one statement so the
        // LLM doesn't read "restricted to {workspace}" and then refuse to
        // touch any allowed_roots path even though file operations there are
        // permitted.
        if self.workspace_only {
...
mod tests {
    ...
    fn powershell_wrapper_is_unwrapped_and_inner_validated() {
        ...
        // (These assertions are Windows-only because the case-insensitive
        // cmdlet matching only kicks in on Windows.)
        #[cfg(target_os = "windows")]
        {
        ...
        // On non-Windows the cmdlets won't case-fold, but the unwrap itself
        // still runs — verify the lowercase variant passes there too.
        #[cfg(not(target_os = "windows"))]
        assert!(p.is_command_allowed(
            r#"powershell -Command "get-childitem | select-object | format-table""#
        ));
    }
