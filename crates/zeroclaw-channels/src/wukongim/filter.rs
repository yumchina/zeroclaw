//! Permission allowlist + @mention parsing.

use super::connection::WkChannelType;
...
///
/// Relies only on the structured `mention` object in the payload JSON:
/// - `mention.all == 1` or `"1"` for @all/@everyone.
/// - `mention.uids` containing the bot's UID.
///
/// Text-based pattern matching is intentionally disabled.
pub fn is_mentioned(bot_uid: &str, payload_json: &serde_json::Value, _content: &str) -> bool {
    if let Some(mention) = payload_json.get("mention") {
        if let Some(all) = mention.get("all")
            && (all.as_u64() == Some(1) || all.as_str() == Some("1"))
        {
            return true;
        }
